...
class CircleProgressClass {		; http://ahkscript.org/boards/viewtopic.php?p=41794#p41794
	...
	__New(Options="") {
		...
		this.BarBGColor := (Options.HasKey("BarBGColor") = 1) ? Options.BarBGColor : ""
		...
		this.TextSize := (Options.HasKey("TextSize") = 1) ? Options.TextSize : 10
		this.TextRendering := (Options.HasKey("TextRendering") = 1) ? Options.TextRendering : 1
		this.TextFont := (Options.HasKey("TextFont") = 1) ? Options.TextFont : "Verdana"
		this.TextStyle := (Options.HasKey("TextStyle") = 1) ? Options.TextStyle : "bold"									; you can use for example  "Bold Italic"
		...
		hbm := CreateDIBSection(this.W+100, this.W+100), hdc := CreateCompatibleDC(), obm := SelectObject(hdc, hbm)
	...
	Update(Percent=0, Text="", BackgroundColor="", centerText="") {
		...
		{
			pBrush:=Gdip_BrushCreateSolid("0x" BackgroundColor)
			;Gdip_FillEllipse(this.G, pBrush, this.BarThickness+1, this.BarThickness+1, this.BarDiameter,this.BarDiameter)			
			Gdip_FillEllipse(this.G, pBrush, this.BarThickness+1+100, this.BarThickness+1, this.BarDiameter,this.BarDiameter)			
		}
		pPen2:=Gdip_CreatePen("0x" this.BarBGColor, this.BarThickness-1)
		;Gdip_DrawArc(this.G, pPen2, Round(this.BarThickness/2)+1,  Round(this.BarThickness/2)+1, this.BarDiameter+this.BarThickness, this.BarDiameter+this.BarThickness, 270, Round(360/100*100))
		;Gdip_DrawArc(this.G, pPen2, Round(this.BarThickness/2)+1+100,  Round(this.BarThickness/2)+1, this.BarDiameter+this.BarThickness, this.BarDiameter+this.BarThickness, 270, Round(360/100*100))
		Gdip_DrawArc(this.G, pPen2, Round(this.BarThickness/2)+1+100,  Round(this.BarThickness/2)+1, this.BarDiameter+this.BarThickness, this.BarDiameter+this.BarThickness, 270, Round(360/100*100))
		...
		if (Percent>0)
			...
			Gdip_DrawArc(this.G, this.pPen, Round(this.BarThickness/2)+1+100,  Round(this.BarThickness/2)+1, this.BarDiameter+this.BarThickness, this.BarDiameter+this.BarThickness, 270, Round(360/100*percent))
		if (Text!="") {
			Options := Trim("x"(this.BarDiameter/2)+100 " y1 h" this.W-this.BarDiameter-2 " Right Vcenter r" this.TextRendering " s" this.TextSize " c" this.TextColor A_Space this.TextStyle)			
			Gdip_TextToGraphics(this.G, Text, Options, this.TextFont)
		}
		if (centerText!="") {
			Options := Trim("x100 y1 w" this.W-2 " h" this.W-2 " Center Vcenter r" this.TextRendering " s" this.TextSize " c" this.TextColor A_Space this.TextStyle)
			Gdip_TextToGraphics(this.G, centerText, Options, this.TextFont)
		}
		...
		; UpdateLayeredWindow(this.hwnd, this.hdc, this.X, this.Y, this.W+100, this.W+100)
		UpdateLayeredWindow(this.hwnd, this.hdc, this.X-100, this.Y, this.W+100, this.W+100)
	}
